...
function defineTileLayerS3(layerName, agsName, op) {
	...
	dojo.declare(layerName, esri.layers.TiledMapServiceLayer,{
			...
			getTileUrl:	function(level, row, col) {
					return "http://nwis-mapper-tiles.s3.amazonaws.com/" + layerName + "/NWIS/_alllayers/" +
...
function getSiteTemplate(siteType) {
	...
	"<b>Site Number: </b>${site_no}<br /><b>Site Name: </b> ${site_name}<br /><b>Agency: </b>${agency}<br /><a href=http://waterdata.usgs.gov/nwis/" + nwisDataType + "?agency_code=${agency}&site_no=${site_no} target='_blank'>Access Data</a>");
		...
		// otherwise process the single sites
			...
			while( nmarkers < markers.length) {
				...
				var jsonStr = "{'grSiteNo':'" + site_no + "'," +  "'grSiteName':'" + site_name + "'," + "'grSiteType':'"
					+ siteType + "'," + "'grSiteStatus':'" + siteStatus + "'," + "'grSiteAgency':'" + agency + "'," + "'grSiteUrl':'" + agency + "&site_no=" + site_no + "'}";
...
function doSearchName(srchCode) {
	...
	if (srchCode == 1 ) {
		locator = new esri.tasks.Locator("http://tasks.arcgisonline.com/ArcGIS/rest/services/Locators/ESRI_Places_World/GeocodeServer");
...
function showAquifer(xml, ioargs){
	...
	var aqHTML = '<table border="0" width="290">' +
					'<colgroup width="80%" />' +
					'<colgroup id="aqColGroup" class="aqColGroup" align="left" valign="left" width="1*" span="2" style=""background:#ddd;" />' +
					'<thead>' +
						'<tr>' +
