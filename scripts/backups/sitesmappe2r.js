...
//check for the two possible urls, which easy need to be proxied seperately
	if (queryURL.indexOf("http://waterdata.usgs.gov") != -1) {
...
function siteCounter(numsites, ioargs) {
	...
	if (siteCount > 0) 
	{
		...
		if (siteCount > 2000)
		{
			fancy_confirm(
				"Warning",
				"Your query had too many results to display, only the extent is shown.  Refine your query parameters in NWIS to display fewer sites.  You can still export or view a site list the results using the 'Export' button on the 'Search Results' panel.");			
		}
...
function draw_NWISweb_box() {

	//this is how you highly a pane
	dijit.byId("acc1").selectChild(dijit.byId("sitesmapper2"));

	//show site count
	document.getElementById("siteCount").innerHTML = "Number of results: " + siteCount + "<br/><br/><i>WARNING: Your query had too many results to display, only the extent is shown.  Refine your query parameters in NWIS to display fewer sites.  You can still export or view a site list the results using the 'Export' button on the 'Search Results' panel.</i>"
	...
	var polygon = esri.geometry.geographicToWebMercator(new esri.geometry.Polygon({"rings": [[[siteData[1], siteData[2]],[siteData[1], siteData[4]],[siteData[3], siteData[4]],[siteData[3], siteData[2]],[siteData[1], siteData[2]]]],"spatialReference": {"wkid": 4326} }));
	var attr = {"numResults":siteCount};
	var infoTemplate = new esri.InfoTemplate("Summary of query","Number of results: ${numResults} <br/><br/><i>WARNING: Your query had too many results to display, only the extent is shown.  Refine your query parameters in NWIS to display fewer sites.  You can still export or view a site list the results using the 'Export' button on the 'Search Results' panel.</i><br/><br/>");   
...
function draw_NWISweb_hull() {
	...
	//show site count
	document.getElementById("siteCountHull").innerHTML = "<br/>Number of results: " + siteCount + "<br/><br/>WARNING: Your query had too many results to display, only the extent is shown.  Refine your query parameters in NWIS to display fewer sites.  You can still export or view a site list the results using the 'Export' button on the 'Search Results' panel.<br/><br/></i>"
	document.getElementById("siteCount").innerHTML = "Number of results: " + siteCount
...
//set up grid
	sitesNWISwebGrid = new dojox.grid.DataGrid({ autoWidth: true, autoHeight: true,
		structure:[
			...
			{name:"Site Name", field:"grSiteName", width:"172px"}
		]
	}, "sitesNWISwebGrid");
