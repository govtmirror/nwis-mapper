...
function drawGLayer(chkBoxId, glayerId) {
	...
	require(["dojo/dom"], function(dom){
		...
		if (glayer) {
			glayer.show();
			if (dom.byId(chkBoxId).checked) {
				glayer.clear();
				drawSitesByType(chkBoxId);
			} else {
				glayer.clear();
				//clearGrid(glayerId.left(6));
				//needed a timeout here after switching to AMD when there are URL params
				setTimeout( function() { clearGrid(glayerId.left(6)); }, 1000);
			}
		}
...
function disableDataTypeRadio(maxIndex) {
	...
	require(["dojo/dom"], function(dom){
		...
		var sp;
		if (maxIndex == 0) {
			sp = dom.byId("sp_sw_act");DisAndGreyOut(sp);
			sp = dom.byId("sp_sw_ina");DisAndGreyOut(sp);
			return;
		}
		if (maxIndex == 1) {
			sp = dom.byId("sp_gw_act");DisAndGreyOut(sp);
			sp = dom.byId("sp_gw_ina");DisAndGreyOut(sp);
			return;
		}
		if (maxIndex == 2) {
			sp = dom.byId("sp_sp_act");DisAndGreyOut(sp);
			sp = dom.byId("sp_sp_ina");DisAndGreyOut(sp);
			return;
		}
		if (maxIndex == 3) {
			sp = dom.byId("sp_at_act");DisAndGreyOut(sp);
			sp = dom.byId("sp_at_ina");DisAndGreyOut(sp);
			return;
		}
		if (maxIndex == 4) {
			sp = dom.byId("sp_ot_act");DisAndGreyOut(sp);
			sp = dom.byId("sp_ot_ina");DisAndGreyOut(sp);
			return;
		}
	});
}
...
function enableDataTypeRadio(maxIndex) {
	...
	require(["dojo/dom"], function(dom){
		...
		var sp;
		if (maxIndex == 0) {
			sp = dom.byId("sp_sw_act");EnableAndBlack(sp);
			sp = dom.byId("sp_sw_ina");EnableAndBlack(sp);
			return;
		}
		if (maxIndex == 1) {
			sp = dom.byId("sp_gw_act");EnableAndBlack(sp);
			sp = dom.byId("sp_gw_ina");EnableAndBlack(sp);
			return;
		}
		if (maxIndex == 2) {
			sp = dom.byId("sp_sp_act");EnableAndBlack(sp);
			sp = dom.byId("sp_sp_ina");EnableAndBlack(sp);
			return;
		}
		if (maxIndex == 3) {
			sp = dom.byId("sp_at_act");EnableAndBlack(sp);
			sp = dom.byId("sp_at_ina");EnableAndBlack(sp);
			return;
		}
		if (maxIndex == 4) {
			sp = dom.byId("sp_ot_act");EnableAndBlack(sp);
			sp = dom.byId("sp_ot_ina");EnableAndBlack(sp);
			return;
		}
...
function defineGraphicLayer(layerName, op) {
	...
	require(["esri/layers/GraphicsLayer"], function(GraphicsLayer) {
		...
		var layerObj = new GraphicsLayer(
...
function defineTileLayerS3(layerName, agsName, op) {

	require(["dojo/_base/declare","dojo/string","esri/layers/TiledMapServiceLayer","esri/SpatialReference","esri/geometry/Extent","esri/layers/TileInfo"], function(declare,string,TiledMapServiceLayer,SpatialReference,Extent,TileInfo){
	
		declare(layerName, TiledMapServiceLayer,{
				...
				getTileUrl:	function(level, row, col) {
						return "http://nwis-mapper-tiles.s3.amazonaws.com/" + layerName + "/" +
...
function defineAquiferLayer(layerName, agsName, op) {
	...
	require(["dojo/_base/declare","dojo/string","esri/layers/TiledMapServiceLayer","esri/SpatialReference","esri/geometry/Extent","esri/layers/TileInfo"], function(declare,string,TiledMapServiceLayer,SpatialReference,Extent,TileInfo){
		...
		declare(layerName, TiledMapServiceLayer,{
			...
			getTileUrl:	function(level, row, col) {
				return "http://maps.waterdata.usgs.gov/mapper/sandbox/dmc/tiles/" + layerName + "/" +
						"L" + string.pad(level, 2, '0') + "/" +
						"R" + string.pad(row.toString(16), 8, '0') + "/" +
						"C" + string.pad(col.toString(16), 8, '0') + "." +
						"png";
...
function getHasDataType(siteType) {
	...
	require(["dojo/dom","dojo/query"], function(dom,query){
		...
		var sp;
		switch (siteType) {
			case "chkSWAct": sp = dom.byId("sp_sw_act"); break;
			case "chkSWIna": sp = dom.byId("sp_sw_ina"); break;
			case "chkGWAct": sp = dom.byId("sp_gw_act"); break;
			case "chkGWIna": sp = dom.byId("sp_gw_ina"); break;
			case "chkSPAct": sp = dom.byId("sp_sp_act"); break;
			case "chkSPIna": sp = dom.byId("sp_sp_ina"); break;
			case "chkATAct": sp = dom.byId("sp_at_act"); break;
			case "chkATIna": sp = dom.byId("sp_at_ina"); break;
			case "chkOTAct": sp = dom.byId("sp_ot_act"); break;
			case "chkOTIna": sp = dom.byId("sp_ot_ina"); break;
		}

		//var hasDataCode = "";
		query('input', sp).forEach (
			function(inputElem) {
...
function getSiteTemplate(siteType) {
	...
	require(["dojo/dom","dojo/query","esri/InfoTemplate"], function(dom,query,InfoTemplate){
		...
		siteTemplate = new InfoTemplate("Site Information",
...
function drawSitesByType(siteType) {
	...
	require(["esri/geometry/Extent","esri/geometry/webMercatorUtils","esri/urlUtils","dojo/request/xhr","esri/domUtils"], function(Extent,webMercatorUtils,urlUtils,xhr,domUtils){
		...
		var bboxExtent = new Extent(map.extent.xmin, map.extent.ymin, map.extent.xmax, map.extent.ymax, map.spatialReference);
		bboxExtent = webMercatorUtils.webMercatorToGeographic(bboxExtent);
		...
		web_proxy_url += "bBox=" + xmin + "," + ymin + "," + xmax +  ","  + ymax;

		// set the format
		web_proxy_url += "&format=mapper,1.0";

		// set the status
		web_proxy_url += "&siteStatus=" + getSiteStatus(siteType);

		// set the site types
		web_proxy_url += "&siteType=" + getSiteTypeUrl(siteType);

		// set the dataType
		web_proxy_url += "&hasDataTypeCd=" + getHasDataType(siteType);

		// set the xhrGet properties
		var urlObj = urlUtils.urlToObject(web_proxy_url);

		// get the load function names
		/*  commented out with new xhr request method
		...
		*/
		...
		switch (siteType) {
			case "chkSWAct":document.getElementById("swActLoader").style.visibility="visible"; break;
			case "chkSWIna":document.getElementById("swInaLoader").style.visibility="visible"; break;
			case "chkGWAct":document.getElementById("gwActLoader").style.visibility="visible"; break;
			case "chkGWIna":document.getElementById("gwInaLoader").style.visibility="visible"; break;
			case "chkSPAct":document.getElementById("spActLoader").style.visibility="visible"; break;
			case "chkSPIna":document.getElementById("spInaLoader").style.visibility="visible"; break;
			case "chkATAct":document.getElementById("atActLoader").style.visibility="visible"; break;
			case "chkATIna":document.getElementById("atInaLoader").style.visibility="visible"; break;
			case "chkOTAct":document.getElementById("otActLoader").style.visibility="visible"; break;
			case "chkOTIna":document.getElementById("otInaLoader").style.visibility="visible"; break;
		}
...
*/
		...
		// newer AMD style AJAX call to fetch sites
		xhr(urlObj.path, {
		handleAs: "xml",
		query: urlObj.query
			}).then(function(xml){
				// Do something with the handled data
				window["setMarkers_" + siteType](xml);

			}, function(err){
				// Handle the error condition
				window["errGet_" + siteType](xml);
			});
		
	});
}

function setMarkers_chkSWAct(xml) {

	require(["dojo/store/Memory","esri/symbols/PictureMarkerSymbol","esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/graphic","dojo/json"], function(Memory,PictureMarkerSymbol,webMercatorUtils,Point,SpatialReference,Graphic,json){
		...
		if (single_sites.length == 0 && colo_sites.length == 0){
			//swActStandby.hide();
			//domUtils.hide("swActStandby");
			document.getElementById("swActLoader").style.visibility="hidden";
			return;
		}
		...
		if (single_sites.length == 1) {
			...
			// otherwise process the single sites			
				...
				sitesSWActStore = new Memory({data:emptySite});
				...
				var icon = new PictureMarkerSymbol("./images/" + siteStatus + "/" + siteType + "_" + siteStatus + icon_postfix + ".png", icon_width, icon_height);
				...
				while( nmarkers < markers.length) {
					...
					// build the symbol
					var point = webMercatorUtils.geographicToWebMercator(new Point(parseFloat(markers[nmarkers].getAttribute("lng")), parseFloat(markers[nmarkers].getAttribute("lat")),  new SpatialReference({ wkid: 4326 })));
					var attr = {"site_no":site_no, "site_name":site_name, "siteTypeText":siteTypeText, "agency":agency };
					var marker = new Graphic(point, icon, attr, siteTemplate);
					...
					var jsonStr = '{"grSiteNo":"' + site_no + '",' +  '"grSiteName":"' + site_name + '",' + '"grSiteType":"' + siteTypeText + '",' + '"grSiteStatus":"' + siteStatus + '",' + '"grSiteAgency":"' + agency + '",' + '"grSiteUrl":"' + agency + '&site_no=' + site_no + '"}';		
					var jsonObj = json.parse(jsonStr);
							
					// append the row to the grid
					sitesSWActStore.put(jsonObj);
					nmarkers++;
				}
			}
		}
...
function setMarkers_chkSWIna(xml) {
	...
	require(["dojo/store/Memory","esri/symbols/PictureMarkerSymbol","esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/graphic","dojo/json"], function(Memory,PictureMarkerSymbol,webMercatorUtils,Point,SpatialReference,Graphic,json){
		...
		if (single_sites.length == 0 && colo_sites.length == 0){
			//swInaStandby.hide();
			document.getElementById("swInaLoader").style.visibility="hidden";
			return;
		}
		...
		if (single_sites.length == 1) {
			...
			// otherwise process the single sites			
				...
				sitesSWInaStore = new Memory({data:emptySite});
...
function setMarkers_chkGWAct(xml) {
	...
	require(["dojo/store/Memory","esri/symbols/PictureMarkerSymbol","esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/graphic","dojo/json"], function(Memory,PictureMarkerSymbol,webMercatorUtils,Point,SpatialReference,Graphic,json){
		...
		if (single_sites.length == 0 && colo_sites.length == 0){
			//gwActStandby.hide();
			document.getElementById("gwActLoader").style.visibility="hidden";
			return;
		}
		...
		if (single_sites.length == 1) {
			...
			// otherwise process the single sites			
				...
				sitesGWActStore = new Memory({data:emptySite});
...
function setMarkers_chkGWIna(xml) {
	...
	require(["dojo/store/Memory","esri/symbols/PictureMarkerSymbol","esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/graphic","dojo/json"], function(Memory,PictureMarkerSymbol,webMercatorUtils,Point,SpatialReference,Graphic,json){
		...
		if (single_sites.length == 0 && colo_sites.length == 0){
			//gwInaStandby.hide();
			document.getElementById("gwInaLoader").style.visibility="hidden";
			return;
		}
		...
		if (single_sites.length == 1) {
			...
			// otherwise process the single sites			
				...
				sitesGWInaStore = new Memory({data:emptySite});
...
function setMarkers_chkSPAct(xml) {
	...
	require(["dojo/store/Memory","esri/symbols/PictureMarkerSymbol","esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/graphic","dojo/json"], function(Memory,PictureMarkerSymbol,webMercatorUtils,Point,SpatialReference,Graphic,json){
		...
		if (single_sites.length == 0 && colo_sites.length == 0){
			//spActStandby.hide();
			document.getElementById("spActLoader").style.visibility="hidden";
			return;
		}
		...
		if (single_sites.length == 1) {
			...
			// otherwise process the single sites			
				...
				sitesSPActStore = new Memory({data:emptySite});
...
function setMarkers_chkSPIna(xml) {
	...
	require(["dojo/store/Memory","esri/symbols/PictureMarkerSymbol","esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/graphic","dojo/json"], function(Memory,PictureMarkerSymbol,webMercatorUtils,Point,SpatialReference,Graphic,json){
		...
		if (single_sites.length == 0 && colo_sites.length == 0){
			//spInaStandby.hide();
			document.getElementById("spInaLoader").style.visibility="hidden";
			return;
		}
		...
		if (single_sites.length == 1) {
			...
			// otherwise process the single sites			
				...
				sitesSPInaStore = new Memory({data:emptySite});
...
function setMarkers_chkATAct(xml) {
	...
	require(["dojo/store/Memory","esri/symbols/PictureMarkerSymbol","esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/graphic","dojo/json"], function(Memory,PictureMarkerSymbol,webMercatorUtils,Point,SpatialReference,Graphic,json){
		...
		if (single_sites.length == 0 && colo_sites.length == 0){
			//atActStandby.hide();
			document.getElementById("atActLoader").style.visibility="hidden";
			return;
		}
		...
		if (single_sites.length == 1) {
			...
			// otherwise process the single sites			
				...
				sitesATActStore = new Memory({data:emptySite});
...
function setMarkers_chkATIna(xml) {
	...
	require(["dojo/store/Memory","esri/symbols/PictureMarkerSymbol","esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/graphic","dojo/json"], function(Memory,PictureMarkerSymbol,webMercatorUtils,Point,SpatialReference,Graphic,json){
		...
		if (single_sites.length == 0 && colo_sites.length == 0){
			//atInaStandby.hide();
			document.getElementById("atInaLoader").style.visibility="hidden";
			return;
		}
		...
		if (single_sites.length == 1) {
			...
			// otherwise process the single sites			
				...
				sitesATInaStore = new Memory({data:emptySite});
...
function setMarkers_chkOTAct(xml) {
	...
	require(["dojo/store/Memory","esri/symbols/PictureMarkerSymbol","esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/graphic","dojo/json"], function(Memory,PictureMarkerSymbol,webMercatorUtils,Point,SpatialReference,Graphic,json){
		...
		if (single_sites.length == 0 && colo_sites.length == 0){
			//otActStandby.hide();
			document.getElementById("otActLoader").style.visibility="hidden";
			return;
		}
		...
		if (single_sites.length == 1) {
			...
			// otherwise process the single sites			
				...
				sitesOTActStore = new Memory({data:emptySite});
...
function setMarkers_chkOTIna(xml) {
	...
	require(["dojo/store/Memory","esri/symbols/PictureMarkerSymbol","esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/graphic","dojo/json"], function(Memory,PictureMarkerSymbol,webMercatorUtils,Point,SpatialReference,Graphic,json){
		...
		if (single_sites.length == 0 && colo_sites.length == 0){
			//otInaStandby.hide();
			document.getElementById("otInaLoader").style.visibility="hidden";
			return;
		}
		...
		if (single_sites.length == 1) {
			...
			// otherwise process the single sites			
				...
				sitesOTInaStore = new Memory({data:emptySite});
...
function setCoLoMarkers(siteType, siteStatus, coloMarkers) {
	...
	require(["esri/geometry/webMercatorUtils","esri/geometry/Point","esri/SpatialReference","esri/symbols/PictureMarkerSymbol","esri/InfoTemplate","esri/graphic"], function(webMercatorUtils,Point,SpatialReference,PictureMarkerSymbol,InfoTemplate,Graphic){
		...
		do {
			...
			// create a point 
			var CurrPoint = webMercatorUtils.geographicToWebMercator(new Point(parseFloat(coloMarkers[i].getAttribute("lng")),parseFloat(coloMarkers[i].getAttribute("lat")) ,  new SpatialReference({ wkid: 4326 })));
			...
			do {
				...
				if ((CurrLat == NextLat) && (CurrLng == NextLng)) {
					...
					var icon = new PictureMarkerSymbol("./images/" + siteStatus + "/multi_" + siteType + "_" + siteStatus + ".png", 42, 40);
					...
					var attr = {"html":html};			
					var siteTemplate = new InfoTemplate(boxHeader, "${html}");
					var marker = new Graphic(CurrPoint, icon, attr, siteTemplate);
					glayer.add(marker);
...
function createMultiMarker(point, htmls, labels, cat_code, nlabels, siteType, siteStatus) {
	...
	require(["esri/symbols/PictureMarkerSymbol","esri/InfoTemplate","esri/graphic"], function(PictureMarkerSymbol,InfoTemplate,Graphic){

		var i;
		var boxHeader = "";
		var html = '<div style="height: 150px; overflow-y: scroll">';
		var icon_width = 42;
		var icon = new PictureMarkerSymbol("./images/" + siteStatus + "/multi_" + siteType + "_" + siteStatus + ".png", icon_width, 40);
...
// error callbacks
function errGet_chkSWAct(error, ioargs) {updateStatus("None", 0);document.getElementById("swActLoader").style.visibility="hidden";}
function errGet_chkSWIna(error, ioargs) {updateStatus("None", 1);document.getElementById("swInaLoader").style.visibility="hidden";}
function errGet_chkGWAct(error, ioargs) {updateStatus("None", 2);document.getElementById("gwActLoader").style.visibility="hidden";}
function errGet_chkGWIna(error, ioargs) {updateStatus("None", 3);document.getElementById("gwInaLoader").style.visibility="hidden";}
function errGet_chkSPAct(error, ioargs) {updateStatus("None", 4);document.getElementById("spActLoader").style.visibility="hidden";}
function errGet_chkSPIna(error, ioargs) {updateStatus("None", 5);document.getElementById("spInaLoader").style.visibility="hidden";}
function errGet_chkATAct(error, ioargs) {updateStatus("None", 6);document.getElementById("atActLoader").style.visibility="hidden";}
function errGet_chkATIna(error, ioargs) {updateStatus("None", 7);document.getElementById("atInaLoader").style.visibility="hidden";}
function errGet_chkOTAct(error, ioargs) {updateStatus("None", 8);document.getElementById("otActLoader").style.visibility="hidden";}
function errGet_chkOTIna(error, ioargs) {updateStatus("None", 9);document.getElementById("otInaLoader").style.visibility="hidden";}
...
function showUSGSOffices() {
	...
	require(["dojo/dom","esri/geometry/Extent","esri/geometry/webMercatorUtils","esri/SpatialReference","esri/geometry/Point","esri/symbols/PictureMarkerSymbol","esri/InfoTemplate","esri/graphic"], function(dom,Extent,webMercatorUtils,SpatialReference,Point,PictureMarkerSymbol,InfoTemplate,Graphic){
	
		clearGLayer("wsc_glayer");
		if (dom.byId('chkWSC').checked == false) {
			return;
		}
		...
		//query the store and iterate over returned items
		usgsStore.query().forEach(function(item){
			...
			point = webMercatorUtils.geographicToWebMercator(new Point(longDD, latDD,  new SpatialReference({ wkid: 4326 })));
			//if (map.extent.intersects(point)) {			
					...
					icon = new PictureMarkerSymbol("./images/" + icon_name + ".png", icon_width, icon_height);

					// build the html popup info if user has not submitted a custom site
					if (url == "") {
						siteTemplate = new InfoTemplate("USGS Water Office Information",
					...
					marker = new Graphic(point, icon, attr, siteTemplate);
...
function zoomURLState(selRegion) {
	...
	require(["esri/geometry/Extent","esri/SpatialReference","esri/geometry/webMercatorUtils"], function(Extent, SpatialReference,webMercatorUtils) {
	
		stateZoomStore.query({identity:selRegion,
			onItem : function(region) {
				...
				var esriExtent = new Extent(west, south, east, north, new SpatialReference({wkid: 4326}));
				setTimeout( function() { 
...
function ZoomInSiteType(maxIndex) {
	...
	if ((ZoomStart >= ClickLevelMax[maxIndex]) && (ZoomEnd >= ClickLevelMax[maxIndex])) {
		...
		var typeList = [["sw_act_glayer",sitesSWActStore,sitesSWActGrid], ["sw_ina_glayer",sitesSWInaStore,sitesSWInaGrid], ["gw_act_glayer",sitesGWActStore,sitesGWActGrid], ["gw_ina_glayer",sitesGWInaStore,sitesGWInaGrid], ["sp_act_glayer",sitesSPActStore,sitesSPActGrid], ["sp_ina_glayer",sitesSPInaStore,sitesSPInaGrid], ["at_act_glayer",sitesATActStore,sitesATActGrid], ["at_ina_glayer",sitesATInaStore,sitesATInaGrid], ["ot_act_glayer",sitesOTActStore,sitesOTActGrid], ["ot_ina_glayer",sitesOTInaStore,sitesOTInaGrid]];
		...
		{ 
			...
			for(var j = 0; j < glayer.graphics.length; j++){
				...
				//for sites out of view, fetch the store and delete the site from it
					...
					//query the store for items out of view
					typeList[k][1].query({grSiteNo:siteNo}).forEach(function(item){
						
						//turns out the identity property is called 'undefined', so remove using this
						typeList[k][1].remove(item.undefined);
			...
			//refresh grids after sites have been deleted
			typeList[k][2].refresh();
...
function centerAndZoomOut() {
	...
	require(["esri/geometry/Extent","esri/geometry/Point"], function(Extent,Point) { 
		...
		var bboxExtent = new Extent(map.extent.xmin, map.extent.ymin, map.extent.xmax, map.extent.ymax, map.spatialReference);
		var mapCenter = new Point(bboxExtent.getCenter());
...
function zoomToFirstExtent() {
	...
	require(["esri/geometry/Point","esri/geometry/webMercatorUtils"], function(Point,webMercatorUtils) { 
		...
		var geometry = new Point(MapX, MapY);
...
function setSliderZoom(zoomLevel) {
	...
	require(["dojo/dom"], function(dom){
		...
		dom.byId("zoomLevelIcon").style.backgroundImage = "url('" + levelIcon + "')";
...
function resetLocSearch() {
	...
	require(["dojo/dom"], function(dom){
		...
		dom.byId("srchBoxPlace").className = "srchBoxTextDis";
		dom.byId("srchBoxPlace").value = "Enter Placename";
		dom.byId("srchBoxSites").className = "srchBoxTextDis";
		dom.byId("srchBoxSites").value = "Enter Site Number(s)";
		placesSelect.reset();
		hucsRegSelect.reset();
...
function doSearchName() {
	...
	require(["dojo/dom","esri/tasks/locator"], function(dom,Locator){
	
		document.getElementById("srchPlacesBar").style.visibility="visible";
	
		var srchText = dom.byId("srchBoxPlace").value;
		var locator = new Locator("http://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer");
		
		// pass the parameters to the geocoding callback
		var address = {'SingleLine': srchText};
		var options = {address:address,outFields:["*"]};

		//start geocode function
...
function placeNameCallBack(geocodeResults) {
	...
	require(["esri/geometry/Extent","esri/geometry/webMercatorUtils"], function(Extent,webMercatorUtils) {

		if (geocodeResults.length > 0) {
			var ptAttr = geocodeResults[0].attributes;
			var esriExtent = new Extent(ptAttr.Xmin, ptAttr.Ymin, ptAttr.Xmax, ptAttr.Ymax, new esri.SpatialReference({wkid: 4326}));
			map.setExtent(webMercatorUtils.geographicToWebMercator(esriExtent));
		} else {
			showErrBox("Unable to Find Placename");
		}

		document.getElementById("srchPlacesBar").style.visibility="hidden";
...
function doSearchSites(siteNoOrName){
	...
	require(["dojo/dom","esri/urlUtils","dojo/request/xhr"], function(dom,urlUtils,xhr){

		// get the sites from the search box
		if (siteNoOrName == "site_num") {
			var srchText = dom.byId("srchBoxSites").value;
		} else {
			var srchText = dom.byId("srchBoxStation").value;
		}
		...
		if (siteNoOrName == "site_num") {
			...
			web_proxy_url += "&station_nm=" + srchText;
		}

		// set the xhrGet properties
		var urlObj = urlUtils.urlToObject(web_proxy_url);
		
		document.getElementById("srchSitesBar").style.visibility="visible";
...
function zoomToSites(xml) {

	require(["esri/geometry/Point","esri/SpatialReference","esri/geometry/Extent","dojo/Deferred"], function(Point,SpatialReference,Extent,Deferred) {
		...
		if (search_sites.length == 0 || xml == null){
			document.getElementById("srchSitesBar").style.visibility="hidden";
		...
		while (nsites < search_sites.length ) {
			...
			var point = new Point(longDD,latDD, new SpatialReference({ wkid: 4326 }));
		...
		maxX = maxX + 0.1;
		maxY = maxY + 0.1;
		var hullExtent = new Extent(minX, minY, maxX, maxY, new SpatialReference({wkid: 4326}));
...
function zoomSearchSite(hull) {
	...
	require(["esri/geometry/webMercatorUtils"], function(webMercatorUtils) {
		...
		map.setExtent(webMercatorUtils.geographicToWebMercator(hull));
...
function showErrBox(errMessage) {
	...
	require(["dijit/registry"], function(registry) {
		...
		var errBox = registry.byId('errExpDialog');
	...
	require(["dijit/registry"], function(registry) {
		...
		if ((stc == 'GW') || (stc == 'GW-CR') ||
			...
			(stc == 'SB-TSM') || (stc == 'SB-UZ')) {
				if (site_status == 'A' || site_status == 'L' || site_status == 'M') {
					cb = registry.byId("chkGWAct");cb.set("checked",true);
				} else {
					cb = registry.byId("chkGWIna");cb.set("checked",true);
				}
				return;
		}
		...
		if (stc == 'SP') {
			if (site_status == 'A' || site_status == 'L' || site_status == 'M') {
				cb = registry.byId("chkSPAct");cb.set("checked",true);
			} else {
				cb = registry.byId("chkSPIna");cb.set("checked",true);
			}
			return;
		}
		...
		if (stc == 'AT') {
			if (site_status == 'A' || site_status == 'L' || site_status == 'M') {
				cb = registry.byId("chkATAct");cb.set("checked",true);
			} else {
				cb = registry.byId("chkATIna");cb.set("checked",true);
			}
			return;
		}
		...
		if ((stc == 'AG') || (stc == 'AS') || (stc == 'AW') ||
			...
			(stc == 'LA-SNK') || (stc == 'LA-SR') || (stc == 'GL')) {
				if (site_status == 'A' || site_status == 'L' || site_status == 'M') {
					cb = registry.byId("chkOTAct");cb.set("checked",true);
				} else {
					cb = registry.byId("chkOTIna");cb.set("checked",true);
				}
			return;
		}
...
function makeStatusGrid() {
	...
	require(["dojo/request","dojo/store/Memory","dgrid/OnDemandGrid"], function (request, Memory, OnDemandGrid) {
		...
		request("json/status.json", {
			...
			// with the data
			statusStore = new Memory({ data: response });
			 
			// Create an instance of OnDemandGrid referencing the store
			statusGrid = new OnDemandGrid({
				store:statusStore,
				columns:[
					{label:"Site Group", field:"grStatusType", width:"5%"},
					{label:"Site Status", field:"grStatusStatus", width:"5%"},
					{label:"Map Symbol", field:"grStatusSymbol", styles:"text-align:center;", formatter:formatMapSymbol, width:"5%"},
					{label:"Selectable Zoom Level", field:"grStatusLevel", width:"5%"},
					{label:"Visible", field:"grStatusVisible", width:"5%"},
					{label:"Selectable", field:"grStatusSelect", formatter:formatSelect, width:"5%"},
					{label:"# Sites on Map", field:"grStatusNSites", width:"10%"}
				]
			}, "statusGrid");
			statusGrid.startup();
		});
...
require(["dojo/_base/declare","dgrid/OnDemandGrid","dgrid/Selection"], function (declare,OnDemandGrid,Selection) {
	...
	//=======================================================================
		...
		sitesSWActGrid = new CustomGrid({
            columns:[
				{label:"Site Number", field:"grSiteNo", width:"5%"},
				{label:"Site Name", field:"grSiteName", width:"10%"},
				//just remove formatter and change json string above in site loop
				//{label:"Site Group", field:"grSiteType", formatter:formatSiteType, width:"5%"},
				{label:"Site Type", field:"grSiteType", width:"5%"},
				{label:"Status", field:"grSiteStatus", formatter:formatSiteStatus, width:"5%"},
				{label:"Agency", field:"grSiteAgency", formatter:formatAgency, width:"5%"},
				{label:"Access Data", field:"grSiteUrl", formatter:formatNWISUrl, width:"5%"}
			],
            selectionMode: "single" // for Selection; only select a single row at a time
        }, "sitesSWActGrid");
		sitesSWActGrid.startup();
		
		//Connect listeners to grid
		sitesSWActGrid.on(".dgrid-row:mouseover", function(event){
			drawSelectHalo("sw_act_glayer", sitesSWActGrid.row(event).data.grSiteNo);
		});
		sitesSWActGrid.on(".dgrid-row:click", function(event){
			drawSelectPopup("sw_act_glayer", sitesSWActGrid.row(event).data.grSiteNo);
		});


		sitesSWInaGrid = new OnDemandGrid({
			columns:[
				{label:"Site Number", field:"grSiteNo", width:"5%"},
				{label:"Site Name", field:"grSiteName", width:"10%"},
				//just remove formatter and change json string above in site loop
	...
	//=======================================================================
		sitesGWActGrid = new OnDemandGrid({
			columns:[
				{label:"Site Number", field:"grSiteNo", width:"5%"},
				{label:"Site Name", field:"grSiteName", width:"10%"},			
				//just remove formatter and change json string above in site loop
		...
		sitesGWInaGrid = new OnDemandGrid({
			columns:[
				{label:"Site Number", field:"grSiteNo", width:"5%"},
				{label:"Site Name", field:"grSiteName", width:"10%"},
				//just remove formatter and change json string above in site loop
	...
	//=======================================================================
		sitesSPActGrid = new OnDemandGrid({
			columns:[
				{label:"Site Number", field:"grSiteNo", width:"5%"},
				{label:"Site Name", field:"grSiteName", width:"10%"},
				//just remove formatter and change json string above in site loop
		...
		sitesSPInaGrid = new OnDemandGrid({
			columns:[
				{label:"Site Number", field:"grSiteNo", width:"5%"},
				{label:"Site Name", field:"grSiteName", width:"10%"},
				//just remove formatter and change json string above in site loop
	...
	//=======================================================================
		sitesATActGrid = new OnDemandGrid({
			columns:[
				{label:"Site Number", field:"grSiteNo", width:"5%"},
				{label:"Site Name", field:"grSiteName", width:"10%"},
				//just remove formatter and change json string above in site loop
		...
		sitesATInaGrid = new OnDemandGrid({
			columns:[
				{label:"Site Number", field:"grSiteNo", width:"5%"},
				{label:"Site Name", field:"grSiteName", width:"10%"},
				//just remove formatter and change json string above in site loop
	...
	//=======================================================================
		sitesOTActGrid = new OnDemandGrid({
			columns:[
				{label:"Site Number", field:"grSiteNo", width:"5%"},
				{label:"Site Name", field:"grSiteName", width:"10%"},
				//just remove formatter and change json string above in site loop
		...
		sitesOTInaGrid = new OnDemandGrid({
			columns:[
				{label:"Site Number", field:"grSiteNo", width:"5%"},
				{label:"Site Name", field:"grSiteName", width:"10%"},
				//just remove formatter and change json string above in site loop
...
function drawSelectPopup(siteLayerId, rowValue) {
	...
	require(["esri/symbols/PictureMarkerSymbol","esri/InfoTemplate","esri/graphic"], function(PictureMarkerSymbol,InfoTemplate,Graphic) { 
		...
		//get site type for template
		var siteType;
		switch (siteLayerId) {
			case "sw_act_glayer": siteType = "chkSWAct"; break;
			case "sw_ina_glayer": siteType = "chkSWIna"; break;
			case "gw_act_glayer": siteType = "chkGWAct"; break;
			case "gw_ina_glayer": siteType = "chkGWIna"; break;
			case "sp_act_glayer": siteType = "chkSPAct"; break;
			case "sp_ina_glayer": siteType = "chkSPIna"; break;
			case "at_act_glayer": siteType = "chkATAct"; break;
			case "at_ina_glayer": siteType = "chkATIna"; break;
			case "ot_act_glayer": siteType = "chkOTAct"; break;
			case "ot_ina_glayer": siteType = "chkOTIna"; break;
		}
		...
		for (var i = 0; i < graphics.length; i++) {
			console.log(rowValue,siteLayerId,graphics[i]);
			if (graphics[i].attributes.site_no == rowValue) {
				
			
				// create the selected icon
				var icon = new PictureMarkerSymbol("./images/selected_site_yellow.png", 64, 64);

				// get the attributes and the template
				var attr = graphics[i].attributes;
				var siteTemplate = getSiteTemplate(siteType);

				// create a selection graphic
				var selGraphic1 = new Graphic(graphics[i].geometry, icon, attr, siteTemplate);
	...
	require(["esri/symbols/PictureMarkerSymbol","esri/InfoTemplate","esri/graphic"], function(PictureMarkerSymbol,InfoTemplate,Graphic) { 
		...
		for (var i = 0; i < graphics.length; i++) {
			if (graphics[i].attributes.site_no == rowValue) {
				...
				// create a selection graphic
				var selGraphic = new Graphic( graphics[i].geometry, icon, attr, siteTemplate);
...
function makeHucSearch() {
	...
	require(["dojo/request","dojo/store/Memory","dijit/form/FilteringSelect"], function (request, Memory,FilteringSelect) {
	
		request("json/regions.json", {
			handleAs: "json"
		}).then(function (response) {
			// Once the response is received, build an in-memory store with the data
			hucsRegStore = new Memory({ data: response });
...
function getHucRegion(selRegion) {
	...
	require(["esri/geometry/Extent","esri/SpatialReference","esri/geometry/webMercatorUtils"], function(Extent,SpatialReference,webMercatorUtils) { 
		...
		//create variable for selection results
		var storeSelection = hucsRegStore.get(selRegion);
		
		//zoom to extent of selection results
		var esriExtent = new Extent(storeSelection.MinX, storeSelection.MinY, storeSelection.MaxX, storeSelection.MaxY, new SpatialReference({wkid: 4326}));
		map.setExtent(webMercatorUtils.geographicToWebMercator(esriExtent));
...
function makeUSGSLayer() {
	...
	require(["dojo/request","dojo/store/Memory"], function (request, Memory) {
		...
		request("json/usgs_water_offices.json", {
			handleAs: "json"
		}).then(function (response) {
			// Once the response is received, build an in-memory store
			// with the data
			usgsStore = new Memory({ data: response });
		});
...
function makePlacesSearch() {
	...
	require(["dojo/request","dojo/store/Memory","dijit/form/FilteringSelect"], function (request, Memory,FilteringSelect) {
		...
		request("json/places.json", {
			handleAs: "json"
		}).then(function (response) {
			// Once the response is received, build an in-memory store with the data
			placesStore = new Memory({ data: response });
...
function getPlace(selRegion) {
	...
	require(["esri/geometry/Extent","esri/SpatialReference","esri/geometry/webMercatorUtils"], function(Extent,SpatialReference,webMercatorUtils) { 
		...
		//create variable for selection results
		var storeSelection = placesStore.get(selRegion);
...
function getAquifer(mapPoint) {
	...
	require(["dojo/dom","dojo/request/xhr","esri/urlUtils"], function (dom,xhr,urlUtils) {

		// only query web service is the checkbox is checked
		// and the zoom level is between 4 and 12
		if (!dom.byId("chkAQ").checked){
			return;
		}
		...
		aqURL = "http://maps.waterdata.usgs.gov/mapper/aquifers/?ptInfo=" +
		map.getLevel() + "," + aqMapX + "," + aqMapY

		// set the xhrGet properties
		var urlObj = urlUtils.urlToObject(aqURL);
...
function setExport(tabCode) {
	...
	require(["dijit/registry"], function(registry){
		...
		var statusTab;
		switch (tabCode) {
			case 1: statusTab = registry.byId("mapSWContainer"); break;
			case 2: statusTab = registry.byId("mapGWContainer"); break;
			case 3: statusTab = registry.byId("mapSPContainer"); break;
			case 4: statusTab = registry.byId("mapATContainer"); break;
			case 5: statusTab = registry.byId("mapOTContainer"); break;
		}
		...
		registry.byId('exportDialog').show();
...
function makeExportFile() {
	...
	require(["dojo/dom","dojo/query","dojo/dom-construct","dojo/_base/window","esri/geometry/Extent","esri/geometry/webMercatorUtils"], function(dom,query,domConstruct,win,Extent,webMercatorUtils) {

		// need to build the URL for the exporter web service
		// var expURL = "http://192.168.2.107/mapper/export/?"
		var expURL = "http://maps.waterdata.usgs.gov/mapper/export/?";
		...
		var expButtons = dom.byId("exportButtons");

		var expFormat = "";
		query('input', expButtons).forEach (
			function(inputElem) {
				if (inputElem.checked) {
	  ...
	  var fmtCode = "";
		...
		if (fmtCode == "1") {
			...
			var dlframe = domConstruct.create("iframe", {src: expURL, style: "display: none"},  win.doc.body);
		}
...
function createBasemapGallery() {
	...
	require(["esri/dijit/BasemapLayer","esri/dijit/Basemap","esri/dijit/BasemapGallery"], function(BasemapLayer,Basemap,BasemapGallery) {
		...
		//add USA topo maps
		var USATopoBasemapLayer = new BasemapLayer({
