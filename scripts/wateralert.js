...
function getSiteTypeUrl(siteType) {
	...
	switch (primaryType) {
//		case "SW": siteTypeUrl = "ES,LK,OC,ST,WE"; break;
        case "SW": siteTypeUrl = "ST"; break;
		case "GW": siteTypeUrl = "GW"; break;
...
function drawSitesByType(siteType) {
	...
	// set the dataType
	web_proxy_url += "&hasDataTypeCd=uv";

    // set the period for current conditions (real time) data
   	web_proxy_url += "&period=P" + rt_days + "D";
   	   
    // set the parameters if non-qw data are requested
    if (getHasDataType(siteType) == "w1") {
      web_proxy_url += "&parameterCd=" + rt_parms;
     }
    
    // set the parameters if qw data are requested
    if (getHasDataType(siteType) == "w2") {
    	web_proxy_url += "&parameterCd=" + rt_qwparms;
    }
