...
function defineTileLayerS3(layerName, agsName, op) {
	...
	dojo.declare(layerName, esri.layers.TiledMapServiceLayer,{
			...
			getTileUrl:	function(level, row, col) {
					return "http://nwis-mapper.s3.amazonaws.com/water-alert/" + layerName + "/" +
...
function defineAquiferLayer(layerName, agsName, op) {
    ...
    dojo.declare(layerName, esri.layers.TiledMapServiceLayer,{
        ...
        getTileUrl:	function(level, row, col) {
            return "./sandbox/dmc/tiles/" + layerName + "/" +
                    "L" + dojo.string.pad(level, 2, '0') + "/" +
                    "R" + dojo.string.pad(row.toString(16), 8, '0') + "/" +
                    "C" + dojo.string.pad(col.toString(16), 8, '0') + "." +
                    "png";
...
function drawSitesByType(siteType) {
    ...
    if (hasDataCode == "w1") {
      web_proxy_url += "&parameterCd=" + pcodes;
     }
    
    // set the parameters if qw data are requested
    if (hasDataCode == "w2") {
    	web_proxy_url += "&parameterCd=" + rt_parms_qw;
    }
	
	//console.log(web_proxy_url);
...
function doSearchSites(siteNoOrName){
	...
	var web_proxy_url = "../nwissitesmapper/nwis/inventory?format=sitefile_output"
